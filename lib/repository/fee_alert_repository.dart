...
class FeeAlertRepository implements Disposable {
  final StreamController<FeeAlert> _alertValueStreamController =
      StreamController<FeeAlert>.broadcast();
  final StreamController<FeeAlert> _alertDeletionStreamController =
      StreamController<FeeAlert>.broadcast();

  Stream<FeeAlert> get alertValueStream => _alertValueStreamController.stream;
  Stream<FeeAlert> get alertDeletionStream =>
    ...
    if (token != null) {
      ...
      _alertDeletionSubscription =
        ...
        if (event.snapshot.value != null) {
          _alertDeletionStreamController
              .add(FeeAlert.fromMap(event.snapshot.value as Map));
        }
      });
      ...
      _alertValueSubscription = alertRef(poolId, token).onValue.listen((event) {
        if (event.snapshot.value != null) {
          ...
          _alertValueStreamController.add(FeeAlert());
        }
      });
    } else {
  ...
  Future<void>? addAlert(StakePool pool) async {
    ...
    if (token != null) {
      Map alert =
          FeeAlert(fixedFee: pool.ff, variableFee: pool.fm, pledge: pool.p)
              .toMap();
